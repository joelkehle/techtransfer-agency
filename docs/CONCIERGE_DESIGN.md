# Concierge Agent — Design Document
...
```
┌──────────────┐          ┌───────────────────────┐          ┌──────────────────┐
│   Browser    │──POST───►│    Concierge Agent     │──bus────►│ Specialist Agents│
...
The concierge is a single process running two things:
...
| Concierge web UI | Other BDOs | Browser → concierge → bus → specialists |

The concierge is the second path. The tto-assistant already has the first.
...
6. As final reports arrive in the concierge's bus inbox:
   - Concierge stores them in memory keyed by submission token
...
## Concierge Endpoints (Browser-Facing)
...
## Concierge as a Bus Agent

The concierge registers on the bus like any other agent:
...
```json
{
  ...
  "mode": "pull",
  "ttl": 120,
  "secret": "<concierge-secret>"
}
```

### Sending Requests

When a BDO submits a document for the "patent-screen" workflow, the concierge:
...
The concierge's poll loop watches its inbox for `response` messages. When one
...
The concierge does not hardcode any workflow. It discovers available specialist
agents from the bus registry by querying capabilities. When someone adds a new
specialist agent to the bus, it appears in the concierge UI automatically.
...
capability name. This keeps the concierge decoupled — it doesn't need a code
...
The concierge does not modify report content. It passes through whatever the
...
The concierge holds submission state in memory:
...
This state is ephemeral. If the concierge restarts, in-flight submissions are
...
The concierge reuses `internal/patentteam/client.go` for all bus communication
...
| 4 | Workflow-agnostic concierge | New specialists can be added to the bus without touching the concierge |
...
| 7 | Capability-based discovery | Concierge finds workflows from bus agent registry, not hardcoded config |
