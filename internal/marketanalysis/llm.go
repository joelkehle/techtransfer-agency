...
const systemPrompt = "You are a technology commercialization analyst conducting a preliminary market assessment for a university tech transfer office. You provide structured, evidence-based analysis. You are conservative in your estimates and explicit about uncertainty. You never fabricate market data — if you don't know, you say so and explain what information would be needed. Respond with strict JSON only."
...
func (a *AnthropicCaller) GenerateJSON(ctx context.Context, prompt string) (LLMResponse, error) {
	...
	ctx, span := otel.Tracer("techtransfer-agency/marketanalysis").Start(ctx, "llm.anthropic.generate")
	defer span.End()
	span.SetAttributes(
